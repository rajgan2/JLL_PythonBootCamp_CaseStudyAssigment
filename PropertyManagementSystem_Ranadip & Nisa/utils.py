...
# Function to read CSV data and display tenant information
def display_tenant_data(file_path):
    with open(file_path, mode='r') as file:
        reader = csv.DictReader(file)
        for row in reader:
            print("Tenant ID:", row['Tenant ID'])
            print("Name:", row['Name'])
            print("Contact Phone:", row['Contact Phone'])
            print("Contact Email:", row['Contact Email'])
            print("Property ID:", row['Property ID'])
            print("-" * 40)
